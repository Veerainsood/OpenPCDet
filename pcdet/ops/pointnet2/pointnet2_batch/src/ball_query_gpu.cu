...
__global__ void ball_query_kernel_fast_pvt(int b, int n, int m, float radius, int nsample, 
...
void ball_query_kernel_launcher_fast_pvt(int b, int n, int m, float radius, int nsample, \
    ...
    ball_query_kernel_fast_pvt<<<blocks, threads>>>(b, n, m, radius, nsample, new_xyz, xyz, idx_cnt, idx);
    // cudaDeviceSynchronize();  // for using printf in kernel function
    err = cudaGetLastError();
    if (cudaSuccess != err) {
        fprintf(stderr, "CUDA kernel failed : %s\n", cudaGetErrorString(err));
