...
class PointHeadTemplate(nn.Module):
    ...
    @staticmethod
    def make_fc_layers(fc_cfg, input_channels, output_channels=None, linear=True):
        fc_layers = []
        c_in = input_channels
    ...
    def assign_stack_targets(self, points, gt_boxes, extend_gt_boxes=None,
                             ...
                             set_ignore_flag=True, use_ball_constraint=False, central_radius=2.0, use_topk=False, topk=1, dim=3):
        """
        Args:
        ...
        point_box_labels = gt_boxes.new_zeros((points.shape[0], self.box_coder.code_size)) if ret_box_labels else None
        ...
        for k in range(batch_size):
            ...
            if use_topk:
                ...
                box_idxs_of_pts[indices] = torch.arange(indices.shape[-1], device=indices.device, dtype=torch.long).unsqueeze(0).repeat(topk, 1)
            else:
            ...
            if ret_box_labels and gt_box_of_fg_points.shape[0] > 0:
                point_box_labels_single = point_box_labels.new_zeros((bs_mask.sum(), self.box_coder.code_size))
